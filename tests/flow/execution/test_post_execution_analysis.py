"""Unit tests for flow/execution/post_execution_analysis.py::PostExecution."""
from types import SimpleNamespace

import pytest
from qiskit import QuantumCircuit

from component.dataclass.job_info import ExecutionResult, JobInfo, MachineExecutionResult
from flow.execution.post_execution_analysis import PostExecution


# ---------------------------------------------------------------------------
# update_job_info_with_results (existing, already-correct code)
# ---------------------------------------------------------------------------
...
def test_update_job_info_marginalizes_merged_distributions_per_job():
    ...
    # Qiskit renders the lowest classical-bit indices on the right. job1 was
    # composed first, so it owns those three rightmost bits.
    assert results["job1"].distribution_no_noise == {"000": 10, "001": 50, "111": 40}
    assert results["job2"].distribution_no_noise == {"00": 10, "01": 20, "10": 30, "11": 40}
...
# ---------------------------------------------------------------------------
# update_machine_info_with_results (newly implemented - was a stub)
# ---------------------------------------------------------------------------
...
def test_update_machine_info_calculates_qubit_time_utilization():
    ...
    scheduler_job_simulation = {
        "machine_a": [
            ...
            SimpleNamespace(
                job_info=[JobInfo(job_name="job3", circuit=QuantumCircuit(4))],
                execution_time=6,
            ),
            SimpleNamespace(
                job_info=[JobInfo(job_name="job4", circuit=QuantumCircuit(9))],
                execution_time=None,
            ),
        ],
    }

    results = PostExecution().update_machine_info_with_results(machines, scheduler_job_simulation)
    ...
    # (2+3)*4 + 4*6 + 9*0 = 44, over capacity(10) * total_time(4+6+0=10) = 100
    assert results["machine_a"].utilization == 0.44
    # Present in `machines` but entirely absent from the simulation dict -
    # still appears in the result, with zero utilization.
...
def test_update_machine_info_rejects_invalid_machine_capacity():
    ...
    with pytest.raises(ValueError, match="positive num_qubits"):
...
def test_update_machine_info_requires_job_qubit_information():
    ...
    scheduler_job_simulation = {
        "machine_a": [
            SimpleNamespace(job_info=[JobInfo(job_name="job1")], execution_time=2),
        ],
    }

    with pytest.raises(ValueError, match="circuit.num_qubits"):
        PostExecution().update_machine_info_with_results(machines, scheduler_job_simulation)


def test_update_machine_info_returns_zero_when_total_execution_time_is_zero():
    machines = {"machine_x": SimpleNamespace(num_qubits=5)}
    scheduler_job_simulation = {
        "machine_x": [
            SimpleNamespace(
                job_info=[JobInfo(job_name="j", circuit=QuantumCircuit(3))],
                execution_time=None,
            ),
        ],
    }
    ...
    assert results["machine_x"].utilization == 0.0


def test_update_machine_info_ignores_simulation_entries_for_machines_not_in_machines_dict():
    machines = {"machine_a": SimpleNamespace(num_qubits=5)}
    scheduler_job_simulation = {
        "machine_a": [],
        "ghost": [
            SimpleNamespace(
                job_info=[JobInfo(job_name="g", circuit=QuantumCircuit(1))],
                execution_time=1,
            ),
        ],
    }
    ...
    assert set(results) == {"machine_a"}
    assert results["machine_a"].utilization == 0.0
